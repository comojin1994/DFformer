...
filename = "shhs_config.yaml"

args = get_configs(config_path=CONFIG_PATH, filename=filename, dataset="SHHS")
...
def load_data():
    ...
    train_dataset = SHHS(args=args, is_test=False, transform=transform)
    ...
    test_dataset = SHHS(args=args, is_test=True, transform=transform)
...
def main():
    ...
    trainer = Trainer(
        ...
        accelerator="gpu",
        devices=devices,
        default_root_dir=f"{args.CKPT_PATH}/shhs",
    )
