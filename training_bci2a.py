...
filename = "bci2a_config.yaml"

args = get_configs(config_path=CONFIG_PATH, filename=filename, dataset="BCIC2a")
...
def load_data(num_subject: int):
    ...
    train_dataset = BCIC2a(args=args, is_test=False, transform=transform)
    ...
    test_dataset = BCIC2a(args=args, is_test=True, transform=transform)
...
def main():
    ...
    for num_subject in range(args.num_subjects):
        ...
        trainer = Trainer(
            ...
            accelerator="gpu",
            devices=devices,
            default_root_dir=f"{args.CKPT_PATH}/bci2a",
        )
